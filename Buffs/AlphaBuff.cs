...
public class AlphaBuff : ModBuff
{
    ...
    public override void Update(Player player, ref int buffIndex)
    {
        ...
        int num;
        for (int i = 0; i < modPlayer.allBuffsIndexes.Count; i += num)
        {
            num = Math.Min(array2.Length, modPlayer.allBuffsIndexes.Count - i);
            for (int j = 0; j < player.buffType.Length; j++)
            {
                ...
                {
                    player.buffTime[j] = (player.buffType[j] = -1);
                }
            }
    ...
    public override void ModifyBuffText(ref string buffName, ref string tip, ref int rare)
    {
        ...
        tip = "";
        tip = tip + ((ModBuff)this).Description.Value + modPlayer.allBuffsIndexes.Count + "\n";
    }
}
